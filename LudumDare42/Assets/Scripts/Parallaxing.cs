...
public class Parallaxing : MonoBehaviour {
    ...
    private float[] parallaxScales;     // The porportion of the camera's movement to move the backgrounds by
    public float smoothing = 1f;        // How smooth the prallax is going to be. Make sure to set this above 0 
                                        // Use this for initialization
    private Transform cam;              // reference to the main camera tranform
    private Vector3 previousCamPos;     // the position of the camera in the previous frame


    // Is called before start(). (but after all game objects are loaded) Great for references.
    private void Awake()
    {
    ...
    void Update()
    {
        ...
        for (int i = 0; i < backgrounds.Length; i++)
        {
            ...
            // fade from current position to target position using lerp
